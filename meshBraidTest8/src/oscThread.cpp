...
void oscThread::update()
{
    ...
    if(leadVertex < tLength){
        ttLength = leadVertex;
    }else if(leadVertex != res){
        ttLength = tLength;
    }
    
    // tail disappears into destination point. That is working. Once the destination is reached,
    // we count backwards at the animation rate until the tail length is 0. Now, once the tail
    // length is 0, we need to remove this instance of oscThread from the vector.
